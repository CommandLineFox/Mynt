...
class MemberArgument {
    ...
    static handleTag(event, message) {
        return Utils_1.tagCheck(message, event.guild.members, (member, tag) => member.user.tag === tag);
    }
    static handleNickname(event, message) {
        return Utils_1.nameCheck(message, event.guild.members, (member, nickname) => member.nickname === nickname);
    }
    static handleUsername(event, message) {
        return Utils_1.nameCheck(message, event.guild.members, (member, username) => member.user.username === username);
    }
